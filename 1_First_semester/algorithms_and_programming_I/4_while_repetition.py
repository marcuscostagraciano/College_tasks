...
def quantidade_de_impares(valor_inicial, valor_final) -> int:
    ...
    """
    count = valor_inicial + 1
    count_impar = 0

    while count < valor_final:
        if count % 2 != 0:
            count_impar += 1
...
def crescimento_populacional(populacao1, populacao2, crescimento1, crescimento2) -> int:
    ...
    """
    count = 0
    cresc_pop1 = crescimento1 / 100
    cresc_pop2 = crescimento2 / 100

    while populacao1 < populacao2:
        populacao1 += populacao1 * (cresc_pop1)
        populacao2 += populacao2 * (cresc_pop2)

        count += 1

    return 0 if count > 100 else count
...
def quantidade_de_primos(inicio, fim) -> int:
...
def serie2(n) -> float:
    ...
    while count <= n:
        soma += count / ((count * 2) - 1)
        count += 1

    return round(soma, 2)


def serie_pi(n) -> float:
