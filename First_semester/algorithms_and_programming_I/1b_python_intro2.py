...
def palindromo(texto) -> bool:
    ...
    Retorna:
        bool: True ou False, dependendo do texto ser palíndromo ou não.
    """
    texto = texto.lower()
    texto = texto.replace("!", "")
    texto = texto.replace(" ", "")
...
def imprime_mes_por_extenso(data) -> str:
    ...
    meses_dic = {1: "janeiro", 2: "fevereiro", 3: "março", 4: "abril",
                 5: "maio", 6: "junho", 7: "julho", 8: "agosto", 9: "setembro",
                 10: "outubro", 11: "novembro", 12: "dezembro"}
    texto = f"{dia} de {meses_dic[mes]} de {ano}"
    return texto


def encontra_caracter(texto, caracter_procurado) -> int:
...
def maior_menor(lista) -> tuple[int, int]:
    ...
    maior = max(lista)
    menor = min(lista)
    maior_e_menor = (maior, menor)
    return maior_e_menor


def media_saltos_lista(saltos) -> float:
    ...
    total_saltos = total_saltos - (max(saltos) + min(saltos))
    total_saltos = total_saltos/(len(saltos)-2)
    return round(total_saltos, 1)


def contem(lista, item_procurado) -> bool:
...
def conta(lista, item_procurado) -> int:
...
def media_temperaturas(temperaturas) -> float:
    ...
    """
    mediaT = sum(temperaturas) / len(temperaturas)
    return mediaT
